...
for (dset in datasets)
{
  ...
  # Construct S4 Object
  sc <- construct(exprmatrix = as.data.frame(counts), samplevector=colnames(counts))

  # Perform PCA
  sc <- doPCA(sc, use.fast = T, use.scaled = F)
  
  # Finally Compute Clusters and Save the ARI
  for (i in 1:n_trials)
  {
    sc <- cluster.assignment(sc, feature.type = "pca", clustering.method="hc", k = nc, rds.dims = 1:10, verbose=F)
    ari[i, dset] <- adjustedRandIndex(cells, getCellMeta(sc, name = "CLUSTER"))
  }
}



write.csv(ari, file = "sincera_results_all.csv")