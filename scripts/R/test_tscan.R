...
datasets <- c("biase", "goolam", "deng1", "deng2", "pollen", "kolodziejczyk", "patel", "fan", "treutlein")
# datasets <- c("deng1")
n_trials <- 100
...
for (dset in datasets)
{
  ...
  for (i in 1:n_trials)
  {
    egtscan <- exprmclust(counts, clusternum = nc, modelNames = "VVV", reduce = T)
    ari[i, dset] <- adjustedRandIndex(cells, egtscan$clusterid)
  }

}

# Write the ARI values to file 
write.csv(ari, file = "tscan_results_all.csv")
